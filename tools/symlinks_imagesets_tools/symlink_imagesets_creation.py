...
# in order to use them for training and testing our systems, avoiding an annoying
...
target_train_file = '5classes_4transf_80-20_train.txt'
target_val_file = '5classes_4transf_80-20_val.txt'
target_train_path = os.path.join(os.getcwd(),'ImageSets', target_train_file)
target_val_path = os.path.join(os.getcwd(),'ImageSets', target_val_file)
...
transformations = ['ROT2D' ,'ROT3D', 'TRANSL', 'SCALE']
...
def write_imageset(view_path):
    ...
    target_train = open(target_train_path, 'a')
    target_val = open(target_val_path, 'a')
    ...
    for filename in os.listdir(view_path):
        if filename.endswith('.jpg'):
            ...
            if((counter%7==0) or (counter%10==0)):
                target_val.write(img_path_noext)
                target_val.write('\n')
            else:
                target_train.write(img_path_noext)
                target_train.write('\n')
            if counter == 100:
                break
            counter = counter+1
...
def create_link(view_path, view_path_an):
    ...
    for filename in os.listdir(view_path):
        if filename.endswith('.jpg'):
            ...
            print real_file_path_an
            if((counter%7==0) or (counter%10==0)):
                linked_file_path = os.path.join(val_images_path, new_image_name)
                linked_file_path_an = os.path.join(val_labels_path, new_annot_name)
                print linked_file_path
                print linked_file_path_an
                ...
                linked_file_path = os.path.join(train_images_path, new_image_name)
                linked_file_path_an = os.path.join(train_labels_path, new_annot_name)
                print linked_file_path
                print linked_file_path_an
                if not os.path.isfile(linked_file_path):
                    os.symlink(real_file_path, linked_file_path)
